...
\begin{document}
    ...
    \paragraph{Exercise 5}
    Suppose $S \in V_\omega$. Pick $n \in \omega$ such that $S \subseteq V_n$. Then $TC\ S \subseteq V_n$
    (since $V_n$ is transitive). Therefore every member of $TC\ S$ is in $V_n$ and hence finite.

    Conversely, suppose $TC\ S$ is finite. By Theorem 9E, $\rank x$ is finite for all $x \in TC\ S$.
    Hence $\rank S = \{ \rank x : x \in TC\ S \}$ is finite, and so $S \in V_\omega$.

    \paragraph{Exercise 6}
    Yes, the replacement axioms are all true in $V_\omega$ by the same argument as the proof of
    Theorem 9L. By the arguments before Theorem 9F, all the axioms of ZFC are true in $V_\omega$
    except the Axiom of Infinity.

    \paragraph{Exercise 7}
    \begin{proof}
        \pf
        \step{1}{Define $h : \omega \rightarrow V_\omega$ by recursion thus: $h(n) = \{ h(m) : m \in g(n) \}$.}
        \step{2}{$h$ is injective.}
        \begin{proof}
            \step{a}{\assume{$h(m) = h(n)$}}
            \step{b}{$g(m) = g(n)$}
            \begin{proof}
                \step{i}{$p \in g(m)$ iff $h(p) \in h(m)$ iff $h(p) \in h(n)$ iff $p \in g(n)$}
            \end{proof}
            \step{c}{$m = n$}
        \end{proof}
        \step{3}{$h$ is surjective.}
        \begin{proof}
            \step{a}{We prove by $\epsilon_{V_\omega}$-induction on $A$ that, for all $A \in V_\omega$,
            there exists $n \in \omega$ such that $h(n) = A$}
            \step{b}{\pflet{$A \in V_\omega$}}
            \step{c}{\assume{$\forall x \in A. \exists n \in \omega. h(n) = x$}}
            \step{d}{\pflet{$m$ be such that $g(m) = \{ n \in \omega : h(n) \in A \}$}}
            \begin{proof}
                \pf\ Since $g$ is surjective.
            \end{proof}
            \step{e}{$h(m) = A$}
        \end{proof}
        \step{4}{If $mEn$ then $h(m) \in h(n)$}
        \qed
    \end{proof}

    \paragraph{Exercise 8}
    Consider the structure $\langle P, R \rangle$ in Exercise 4 of Chapter 7. Then $\langle P, R \rangle
    \in V_\omega$ but its ordinal $\omega^2$ is not.
    ...
    \paragraph{Exercise 10}
    We first prove $S$ is a transitive set. Let $x \in y \in S$. If $y \in \omega$ we have $x \in \omega$
    hence $x \in S$. Otherwise, let $y \in \mathcal{P}^n \omega$ where $n \geq 1$. Then $y \subseteq
    \mathcal{P}^{n-1} \omega$ and so $x \in \mathcal{P}^{n-1} \omega$, hence $x \in S$.

    Now:
    \subparagraph{Axiom of Extensionality}
    $S$ models this because $S$ is transitive (see p. 250).

    \subparagraph{Empty Set Axiom}
    $S$ models this because $\emptyset \in S$.

    \subparagraph{Axiom of Pairing}
    Let $a, b \in S$. Pick $n$ such that $a, b \in \mathcal{P}^n S$. Then $\{ a,b \} \in \mathcal{P}^{n+1} S$
    hence $\{ a,b \} \in S$.

    \subparagraph{Axiom of Union}
    Let $A \in S$.

    If $A \in \omega$ or $x \in \mathcal{P} \omega$ then for all $x \in y \in A$ we have $x \in \omega$ since $\omega$ is transitive,
    hence $\bigcup A \in \mathcal{P} \omega$.

    If $A \in \mathcal{P}^{n+2} \omega$ then for all $x \in y \in A$ we have $x \in \mathcal{P} \omega$,
    hence $\bigcup A \in \mathcal{P}^{n+1} \omega$.

    Thus in all cases $\bigcup A \in S$.

    \subparagraph{Power Set Axiom}
    Let $A \in S$. 

    If $A \in \omega$ then $A \subseteq \omega$, and so every subset of $A$ is a subset of $\omega$.
    Hence $\mathcal{P} A \in \mathcal{P}^2 \omega$.

    If $A \in \mathcal{P}^{n+1} \omega$ then $A \subseteq \mathcal{P}^n \omega$, and so every subset of $A$
    is a subset of $\mathcal{P}^n \omega$. Hence $\mathcal{P} A \in \mathcal{P}^{n+2} \omega$.

    In all cases $\mathcal{P} A \in S$.

    \subparagraph{Subset Axioms}
    Let $A \in S$. Let $\phi$ be a formula not containing $B$. Let $B = \{ x \in A \mid \phi^S \}$.
    Then $B \subseteq A$.

    If $A \in \omega$ then $B \subseteq A \subseteq \omega$ so $B \in \mathcal{P} \omega$.

    If $A \in \mathcal{P}^{n+1} \omega$ then $B \subseteq A \subseteq \mathcal{P}^n \omega$
    hence $B \in \mathcal{P}^{n+1} \omega$.

    In either case $B \in S$.

    \subparagraph{Axiom of Infinity}
    Holds because $\omega \in S$.

    \subparagraph{Axiom of Choice}
    Holds for the same reason as the Subset Axioms.

    \subparagraph{Regualrity Axiom}
    Let $A \in S$ be nonempty. Let $m \in A$ have minimum rank. Then $m \in S$ because $S$ is transitive
    and $m \cap A = \emptyset$.

    \paragraph{Exercise 11}
    Let $\kappa$ be an inaccessible cardinal. We have
    \begin{align*}
        \beth_\kappa & = \bigcup_{\alpha \in \kappa} \beth_\alpha \\
        & \leq \kappa
    \end{align*}
    by Lemma 9K (a). And conversely $\kappa \leq \beth_\kappa$ because $\alpha \leq \beth_\alpha$
    for every ordinal $\alpha$. So $\beth_\kappa = \kappa$.

    Now,
    \begin{align*}
        |V_\kappa| & = |\bigcup_{\alpha \in \kappa} V_\alpha| \\
        & \leq \kappa \cdot \kappa & (\text{Lemma 9K (b)}) \\
        & = \kappa
    \end{align*}
    and
    \begin{align*}
        |V_\kappa| & = |\bigcup_{\alpha \in \kappa} V_\alpha| \\
        & \geq \sup_{\alpha \in \kappa} |V_{\omega + \alpha}| \\
        & = \sup_{\alpha \in \kappa} \beth_\alpha \\
        & = \beth_\kappa \\
        & = \kappa
    \end{align*}
    So $|V_\kappa| = \kappa$.
    ...
    \paragraph{Exercise 12}
    If $\alpha = 0$ then $\alpha$ is the strict supremum of $\emptyset$.

    For any ordinal $\alpha$, we have $\alpha^+$ is the strict supremum of $\{\alpha\}$.

    For $\lambda$ a limit ordinal, let $S$ be a set of smaller ordinals of size $\cf \lambda$ such that
    $\lambda = \sup S$. Then $\lambda = \operatorname{ssup} S$.

    \paragraph{Exercise 13}
    Let $\lambda$ be a limit ordinal. Pick a sequence $S$ of ordinals $< \lambda$ whose supremum is $\lambda$.
    Then
    \[ \beth_\lambda = \sup_{\alpha \in S} \beth_\alpha \]
    and so $\cf \beth_lambda \leq \cf \lambda$.

    Conversely, let $A$ be a sequence of ordinals $< \beth_\lambda$ whose supremum is $\beth_\lambda$.
    Let
    \[ B = \{ \gamma \in \lambda \mid \exists X \in A. \card X \leq \beth_\gamma \} \enspace . \]
    Then $\card B \leq \card A$. To complete the proof it suffices to show that $\sup B = \lambda$.
    Any $\alpha \in A$ has cardinality at most $\beth_{\sup B}$, so $\alpha \in \beth_{\sup B + 1}$.
    Hence $\beth_\lambda = \sup A \leq \beth_{\sup B + 1}$, and so $\lambda \underline{\in} \sup B + 1$.
    Since $\lambda$ is a limit ordinal, $\lambda \underline{\in} \sup B$, whence equality holds.

    \paragraph{Exercise 14}
    The proof is by transfinite induction on $y$ over $R$. Assume that, for all $y' R y$, we have
    $\card \{ x \mid x R^t y' \} < \kappa$. Then $\card \bigcup_{y' R y} \{ x \mid x R^t y' \} < \kappa$
    by Theorem 9T. Hence
    \begin{align*}
        \card \{ x \mid x R^t y \} & = \card (\{ x \mid x R y \} \cup \bigcup_{y' R y} \{ x \mid x R^t y' \}) \\
        & < \card \{ x \mid x R y \} + \card \bigcup_{y' R y} \{ x \mid x R^t y' \} \\
        & < \kappa + \kappa 
        & = \kappa
    \end{align*}

    \paragraph{Exercise 15}
    Let $\kappa$ be an inaccessible cardinal. Then
    \[ \cf \aleph_\kappa = \cf \kappa = \kappa \]
    by Theorem 9N. Also,
    \begin{align*}
        \kappa & \leq \aleph_\kappa \\
        & \leq \beth_\kappa \\
        & = \kappa & (\text{Exercise 11})
    \end{align*}
    Hence $\cf \aleph_\kappa = \aleph_\kappa$ as required.

    \paragraph{Exercise 16}
    Suppose $\lambda$ is weakly inaccessible, so
    \[ \cf \aleph_\lambda = \lambda = \aleph_\lambda \enspace . \]
    Then $\lambda$ is a regular cardinal (since $\lambda = \aleph_\lambda$) and $\lambda \neq \aleph_0$
    (since $\aleph_\omega \neq \aleph_0$).

    Also if the generalized continuum hypothesis holds, then $\lambda = \aleph_\lambda = \beth_\lambda$.
    So for $\alpha < \lambda$, we have $\alpha < \beth_\lambda$, hence $\alpha < \beth_\beta$ for some
    $\beta < \lambda$. Therefore $2^\alpha < \beth_{\beta^+} < \beth_\lambda = \lambda$.

    Thus, for all $\alpha < \lambda$, we have $2^\alpha < \lambda$. So $\lambda$ is an inaccessible cardinal.

    \paragraph{Exercise 17}
    Assume for a contradiction $\card \bigcup_{i \in I} A_i \geq \card \times_{i \in I} B_i$.
    Pick a surjective function $f : \bigcup_{i \in I} A_i \rightarrow \times_{i \in I} B_i$.
    For all $i \in I$, the function $g_i : A_i \rightarrow B_i$ defined by $g_i(x) = f(x)(i)$ cannot be
    surjective. Pick $b_i \in B_i - \ran g_i$ for all $i \in I$. Then $b \in \times_{i \in I} B_i$
    but $b \neq f(x)$ for any $x \in \bigcup_{i \in I} A_i$.

    \paragraph{Exercise 18}
    It is not monotone because $\cf \aleph_0 = \aleph_0$ but $\cf (\aleph_0 + 1) = 1$.

    It is not continuous because $\cf \aleph_\omega = \omega$ but $\sup_{\alpha < \aleph_\omega}
    = \sup \{ \aleph_n : n \in \omega \} = \aleph_\omega$.

    So it is not normal.

    \paragraph{Exercise 19}
    Let $T = \{ \alpha \in \kappa : x \subseteq V_\alpha \}$. Then $T$ is a set of fewer than $\kappa$
    ordinals smaller than $\kappa$, hence $\sup T < \kappa$. We have $S \subseteq V_{\sup T + 1}$ so
    $S \in V_\kappa$.

    \paragraph{Exercise 20}
    (Following the proof of Theorem 9N.)

    First of all we claim that $\cf t_\lambda \leq \cf \lambda$. We know that $\lambda$ is the supremum of
    some set $S \subseteq \lambda$ with $\card S = \cf \lambda$. It suffices to show that $t_\lambda =
    \sup \{ t_\alpha \mid \alpha \in S \}$. But this is Theorem 8E.

    Second, we claim that $\cf \lambda \leq \cf t_\lambda$. Suppose that $t_\lambda$ is the supremum of some
    set $A$ of smaller ordinals. Let
    \[ B = \{ \gamma \in \lambda \mid \exists \alpha \in A. \alpha \leq t_\gamma \} \enspace . \]
    Then $\card B \leq \card A$. To complete the proof it suffices to show that $\sup B = \lambda$. Any
    $\alpha$ in $A$ has cardinality at most $t_{\sup B}$, so $\alpha \in t_{(\sup B) + 1}$. Hence $t_\lambda
    = \sup A \leq t_{(\sup B) + 1}$ and so $\lambda \underline{\in} (\sup B) + 1$. Since $\lambda$ is a
    limit ordinal, $\lambda \underline{\in} \sup B$, whence equality holds.
\end{document}
