...
\begin{document}
    ...
    \begin{theorem}[Transfinite Recursion Theorem, Preliminary Form]
        Assume that $<$ is a well ordering on $A$, and that $G : B^{< A} \rightarrow A$,
        where $B^{< A} = \{ f \mid \exists t \in A. f : \seg t \rightarrow B\}$. Then there exists a unique
        function $F : A \rightarrow B$ such that
        \[ \forall t \in A. F(t) = G(F \restriction \seg t) \enspace . \]
    \end{theorem}
